...
# Heavy sensor streams, by ROS topic suffix. Three RGBD cameras publish a
# 640x480 point cloud and a depth image each, and the bridge carries all six
# whether or not anything subscribes. Mapping needs none of them: it needs
# /scan. P-20 already measured what a single cloud does to an executor, and
# P-47 has three independent consumers of /scan_filtered stopping in the same
# second with Ignition itself healthy - the bridge is the shared suspect.
#
# The light profile is DERIVED from bridge.yaml rather than copied into a
# second file on purpose: a second copy drifts, and a drifted bridge fails by
# silently not carrying a topic, which is the hardest kind of fault to see.
HEAVY_SUFFIXES = ('/points', '/depth_image')


def _light_bridge_config(full_config, out_dir):
    """Write a copy of the bridge config with the heavy camera streams removed.

    Returns the path written, or the original path if anything goes wrong -
    falling back to the full bridge costs performance, while falling back to
    nothing costs the run.
    """
    try:
        import yaml
        with open(full_config, encoding='utf-8') as handle:
            entries = yaml.safe_load(handle) or []
        kept = [entry for entry in entries
                if not str(entry.get('ros_topic_name', '')).endswith(HEAVY_SUFFIXES)]
        os.makedirs(out_dir, exist_ok=True)
        out_path = os.path.join(out_dir, 'bridge_light.yaml')
        with open(out_path, 'w', encoding='utf-8') as handle:
            handle.write('# Generated by sim.launch.py from bridge.yaml - do not edit.\n')
            handle.write('# %d of %d topics kept; %s dropped for the mapping phase.\n'
                         % (len(kept), len(entries), ', '.join(HEAVY_SUFFIXES)))
            yaml.safe_dump(kept, handle, default_flow_style=False, sort_keys=False)
        return out_path
    except Exception as exc:  # noqa: BLE001 - a launch file must not die here
        print('sim.launch.py: could not build the light bridge config (%s); '
              'using the full one' % exc, file=sys.stderr)
        return full_config
...
def generate_launch_description():
    ...
    # heavy_sensors:=false drops the three point clouds and three depth images
    # from the bridge.
    #
    # This is a MEASURING INSTRUMENT first: P-47 narrows the /scan_filtered
    # stall to sensor / bridge / DDS, and running the same scenario with and
    # without the six heavy streams separates the bridge from the other two in
    # one pair of runs.
    #
    # It is NOT safe for a mission run: measure_box reads /camera/points and the
    # V4 grasp closes on the wrist cameras' depth. Mapping uses neither - it
    # uses /scan - so only the mapping phase may ask for it, and a run that
    # maps and then picks needs the clouds back before the mission starts.
    heavy_sensors = LaunchConfiguration('heavy_sensors', default='true')
    heavy_sensors_arg = DeclareLaunchArgument(
        'heavy_sensors', default_value='true',
        description='Bridge the RGBD point clouds and depth images. false '
                    'leaves /scan and the colour images only - mapping and '
                    'P-47 diagnosis only, the grasp needs them.')
    ...
    spawn_args = [
        ...
        # A different layout is a different world file; scripts/gen_world.py
        # writes them from config/world.yaml. The world NAME inside the file has
        # to stay 'seminar_world' either way, because bridge.yaml embeds it in
        # the fingertip contact topics.
        DeclareLaunchArgument(
            'world',
            default_value=os.path.join(pkg_bringup, 'worlds', 'seminar_world.sdf'),
    ...
    # Same node twice, with complementary conditions, because the config file is
    # a plain parameter and cannot be chosen by a substitution.
    bridge_light_config = _light_bridge_config(
        bridge_config, os.path.expanduser('~/.ros/pas_dual_arm'))
    node_ros_gz_bridge = Node(
        ...
        condition=IfCondition(heavy_sensors),
        output=bg_output
    )
    node_ros_gz_bridge_light = Node(
        package='ros_gz_bridge',
        executable='parameter_bridge',
        parameters=[{
            'config_file': bridge_light_config,
            'qos_overrides./tf_static.publisher.durability': 'transient_local',
        }],
        condition=UnlessCondition(heavy_sensors),
        output=bg_output
    )
