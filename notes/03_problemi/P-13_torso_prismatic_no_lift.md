...
|---|---|---|---|---|
| 1 | 30. 6. | efort limit 100 → 1000 N | bez promjene | nije samo efort |
| 2 | 30. 6. | `min`/`max` na command interfaceu | bez promjene | — |
| 3 | 30. 6. | `position_proportional_gain` (sada 20) | bez promjene | — |
...
| 6 | 15. 9. | **`position` sučelje na `main`-u** (Codexov `effort` odbačen), `table_ready` zada 0.200 m preko `torso_controller`, ruke u `table_arms` pozi, GUI | akcija `SUCCEEDED`, stvarno **0.0500 / 0.0500 m** — nula pomaka | potvrđen C1: pozicijsko sučelje pod teretom ne pomiče vodilice **uopće**, ne samo djelomično |
